...
export default function Exchange({ exchanges, onAdd, onDelete, onAddTx }) {
  ...
  const handleSave = async () => {
    ...
    // Auto-add to expenses if checked
    if (addToExpenses && dir === 'sell') {
      await onAddTx({
        type: 'expense',
        amount: kzt,
        date,
        ...
        category: 'other',
        person,
        note: `💱 Обмен ${kzt.toLocaleString('ru-KZ')} ₸ → ${foreign.toFixed(2)} ${sym}${note ? ' · ' + note : ''}`,
        fromExchange: true,
        grossAmount: null,
        commission: null,
      });
      showToast(`Обмен сохранён и добавлен в расходы: −${fmt(kzt)}`);
    } else {
      ...
      <div className="card">
        ...
        {dir === 'sell' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px',
            background: '#E1F5EE', borderRadius: 8, marginBottom: 12, cursor: 'pointer' }}
            onClick={() => setAddToExpenses(!addToExpenses)}>
            <div style={{ width: 20, height: 20, borderRadius: 4, border: '2px solid #1D9E75',
              background: addToExpenses ? '#1D9E75' : 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {addToExpenses && <span style={{ color: '#fff', fontSize: 12 }}>✓</span>}
            </div>
            <span style={{ fontSize: 13, color: '#085041' }}>
              Добавить {fromVal ? fmt(parseFloat(fromVal) || 0) : 'сумму ₸'} в расходы автоматически
            </span>
          </div>
        )}
