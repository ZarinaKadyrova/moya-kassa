...
export default function History({ txns, customCats, month, year, onMonthChange, onDelete, onEdit }) {
    ...
    <>
      ...
      <div className="card">
        {filtered.length === 0
          ...
          : filtered.map(tx => <TxItem key={tx.id} tx={tx} customCats={customCats} onDelete={onDelete} onEdit={onEdit} />)
        }
      </div>
    </>
  );
