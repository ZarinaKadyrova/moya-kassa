...
export default function Overview({ txns, customCats, month, year, onMonthChange, onDelete, onEdit }) {
      ...
      <div className="card">
        ...
        {monthTxns.length === 0
          ...
          : monthTxns.slice(0, 10).map(tx => (
            <TxItem key={tx.id} tx={tx} customCats={customCats} onDelete={onDelete} onEdit={onEdit} />
          ))
        }
      </div>
    </>
